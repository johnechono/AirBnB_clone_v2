#!/usr/bin/python3
"""runs Tests for the AirBnb clone modules.
"""
import os
...
def clear_stream(stream: TextIO):
    """will Clear the contents of a given stream
...
def delete_file(file_path: str):
    """will Remove a file if it exists.
    Args:
...
def reset_store(store: FileStorage, file_path='file.json'):
    """will Reset the items in the given store.
    Args:
...
def read_text_file(file_name):
    """will Read the contents of a given file.
...
def write_text_file(file_name, text):
    """will Write a text to a given file.
